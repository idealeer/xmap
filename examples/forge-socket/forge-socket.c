...
struct config {
    int read_timeout; // how long to wait once connected for the banner
                      // (seconds)
    int                 current_running;
    int                 max_concurrent;
    ...
    struct stats_st {
        int init_connected_hosts; // Number of hosts we have even tried to
                                  // connect to
        ...
        int timed_out;      // # hosts that timed out at all
                            // (conn_timed_out+read_timed_out)?
...
void print_status(evutil_socket_t fd, short events, void *arg) {
    ...
    log_info("forge-socket",
             "(%d/%d in use) - Totals: %d inited, %d connected, %d conn "
             "timeout, %d read timeout %d completed",
             conf->current_running, conf->max_concurrent,
             conf->stats.init_connected_hosts, conf->stats.connected_hosts,
             conf->stats.conn_timed_out, conf->stats.read_timed_out,
             conf->stats.completed_hosts);
}

void decrement_cur_running(struct state *st) {
    struct config *conf = st->conf;
    conf->current_running--;
    log_debug("forge-socket", "done, down to %d", conf->current_running);
...
int set_sock_state(int sock, struct tcp_state *st) {
    ...
    if (setsockopt(sock, IPPROTO_TCP, TCP_STATE, st, sizeof(struct tcp_state)) <
        0) {
...
void grab_banner(struct state *st) {
    ...
    if (st->conf->send_str) {
        ...
        evbuffer_add_printf(evout, st->conf->send_str, inet_ntoa(addr.sin_addr),
    ...
    log_trace("forge-socket", "go %s go! read a byte!!",
              inet_ntoa(addr.sin_addr));
}
...
void stdin_readcb(struct bufferevent *bev, void *arg) {
    ...
    log_debug("forge-socket", "stdin cb %d < %d ?", conf->current_running,
              conf->max_concurrent);
    ...
    while (conf->current_running < conf->max_concurrent &&
        ...
        // synack, 77.176.116.205, 141.212.121.125, 443, 49588, 3628826326,
        // 3441755636, 0, 0,2013-08-11 19:16:05.799
        char     synack[12];
        ...
        int ret = sscanf(
            line, "%11[^,], %15[^,], %15[^,], %hu, %hu, %u, %u, %d, %d,%*s",
            synack, srcip, dstip, &sport, &dport, &seq, &seq_ack, &cooldown,
            &repeat);

        log_trace("forge-socket",
                  "%d '%s' sip: '%s', dip: '%s', sport: %d, dport: %d, seq: "
                  "%d, seq_ack: %d",
...
int main(int argc, char *argv[]) {
    ...
    struct timeval     status_timeout = {1, 0};
    int                c;
    struct option long_options[] = {{"concurrent", required_argument, 0, 'c'},
    ...
    ret =
        ulimit(4, 1000000); // Allow us to open 1 million fds (instead of 1024)
    if (ret < 0) {
    ...
    while (1) {
        int option_index = 0;
        c                = getopt_long(argc, argv, "c:t:r:v:f:d:", long_options,
                        &option_index);
        ...
        case 'f':
            if (strcmp(optarg, "hex") == 0) {
                ...
                log_fatal(
                    "forge-socket",
                    "Unknown format '%s'; use 'hex', 'base64', or 'ascii'",
                    optarg);
            }
            break;
        case 'd':
            fp = fopen(optarg, "r");
            if (!fp) {
                log_error("forge-socket",
                          "Could not open send data file '%s':", optarg);
                perror("fopen");
                exit(-1);
            }
            ...
            if (!conf.send_str) {
                log_fatal("forge-socket", "Could not malloc %d bytes",
                          conf.send_str_size + 1);
            }
            if (fread(conf.send_str, conf.send_str_size, 1, fp) != 1) {
                log_fatal("forge-socket",
                          "Couldn't read from send data file '%s':", optarg);
            }
        ...
        case '?':
            ...
            printf("\t%s [-c max_concurrency] [-r read_timeout] \n\t"
                   "[-v verbosity=0-5] [-d send_data_file] [-f "
                   "ascii|hex|base64]\n",
                   argv[0]);
            exit(1);
        default:
