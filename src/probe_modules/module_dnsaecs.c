    ...
    for (int i = 0; i < num_domains; i++) {
        ...
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_aecs[i] + sizeof(dns_header) +
                                   qname_lens_aecs[i]);
        char *option_qname_p =
            (char *) (dns_packets_aecs[i] + sizeof(dns_header) +
                      qname_lens_aecs[i] + sizeof(dns_question_tail));
        dns_option_tail *option_tail_p =
            (dns_option_tail *) (dns_packets_aecs[i] + sizeof(dns_header) +
                                 qname_lens_aecs[i] +
                                 sizeof(dns_question_tail) +
                                 default_option_qname_len_aecs);
        dns_option_ecs *option_ecs_p =
            (dns_option_ecs *) (dns_packets_aecs[i] + sizeof(dns_header) +
                                qname_lens_aecs[i] + sizeof(dns_question_tail) +
                                default_option_qname_len_aecs +
        ...
        memcpy(qname_p, qnames_aecs[i], qname_lens_aecs[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_aecs[i]);
        ...
        option_tail_p->udpsize = htons(default_option_udpsize_aecs);
...
static uint16_t get_name_helper_aecs(const char *data, uint16_t data_len,
...
int dnsaecs_thread_init(void *buf, macaddr_t *src, macaddr_t *gw,
                        void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_aecs == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth_header(eth_header, src, gw);

    struct ip *ip_header = (struct ip *) (&eth_header[1]);
    uint16_t   ip_len =
        sizeof(struct ip) + sizeof(struct udphdr) + dns_packet_lens_aecs[0];
    make_ip_header(ip_header, IPPROTO_UDP, ip_len);

    struct udphdr *udp_header = (struct udphdr *) (&ip_header[1]);
    uint16_t       udp_len    = sizeof(struct udphdr) + dns_packet_lens_aecs[0];
    make_udp_header(udp_header, udp_len);

    char *payload                = (char *) (&udp_header[1]);
    module_dnsaecs.packet_length = sizeof(struct ether_header) +
                                   sizeof(struct ip) + sizeof(struct udphdr) +
                                   dns_packet_lens_aecs[0];
    assert(module_dnsaecs.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_aecs[0], dns_packet_lens_aecs[0]);
...
int dnsaecs_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    port_h_t src_port = get_src_port(dns_num_ports_aecs, probe_num, validation);
    uint16_t dns_txid = get_dnsa_txid(validation, probe_num);

    if (label_type_aecs == DNS_LTYPE_RAW || label_type_aecs == DNS_LTYPE_STR) {
        // For num_questions_aecs == 1, we handle this in per-thread init. Do
        // less work
        if (num_questions_aecs > 1) {
            ...
            *buf_len      = sizeof(struct ether_header) + sizeof(struct ip) +
            ...
            memcpy(payload, dns_packets_aecs[index],
                   dns_packet_lens_aecs[index]);
        }
        ...
        case DNS_LTYPE_RANDOM: {
            ...
            dns_random_bytes_aecs(new_label, 8, charset_alpha_lower_aecs, 26,
                                  aes);
            new_label[8] = '\0';
            break;
        }
...
void dnsaecs_print_packet(FILE *fp, void *packet) {
    ...
    uint16_t bytes_consumed = 0;
    char *question_name = get_name_aecs(data, data_len, (char *) dns_header_p,
    ...
    dns_option_tail *option_tail_p =
        (dns_option_tail *) (option_qname_p + default_option_qname_len_aecs);
    ...
    fprintf(
        ...
        question_name,
        qtype_strs_aecs[qtype_qtype_to_strid_aecs[(uint16_t) tail_p->qtype]],
...
int dnsaecs_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip_hdr->ip_p == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_aecs, validation)) {
            return PACKET_INVALID;
        }
...
void dnsaecs_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
    ...
    if (ip_header->ip_p == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_aecs(
        ...
        for (int i = 0; i < ntohs(dns_header_p->ancount) && !err; i++) {
            err = process_response_answer_aecs(
                &data, &data_len, (char *) dns_header_p, udp_len, list);
        }
        ...
        // packet prevent this.
        log_fatal("dnsaecs", "Die. This can only happen if you "
