...
void dnsaecsv_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
    ...
    if (ip_header->ip_p == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_aecsv(
        ...
        for (int i = 0; i < ntohs(dns_header_p->ancount) && !err; i++) {
            err = process_response_answer_aecsv(
                &data, &data_len, (char *) dns_header_p, udp_len, list);
        }
        ...
        // packet prevent this.
        log_fatal("dnsaecsv", "Die. This can only happen if you "
...
probe_module_t module_dnsaecsv = {
    ...
    .port_args       = 1,
    .global_init     = &dnsaecsv_global_init,
    .thread_init     = &dnsaecsv_thread_init,
    .make_packet     = &dnsaecsv_make_packet,
    .print_packet    = &dnsaecsv_print_packet,
    .validate_packet = &dnsaecsv_validate_packet,
    .process_packet  = &dnsaecsv_process_packet,
    .close           = &dnsaecsv_global_cleanup,
    ...
    .helptext =
        ...
        "Besides, ECS with variable src_ip/24/0 is appended to the tail.\n"
