 ...
 * with: dns_qtype (.h) qtype_strid_to_qtype_6af (below)
 * qtype_qtype_to_strid_6af (below, and setup_qtype_str_map_6af())
 */
...
static bool process_response_answer_6af(char **data, uint16_t *data_len,
    ...
    if (type > MAX_QTYPE || qtype_qtype_to_strid_6af[type] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(afs, "type_str",
                      (char *) qtype_strs_6af[qtype_qtype_to_strid_6af[type]],
                      0);
    }
...
static int load_question_from_str_6af(const char *type_q_str) {
    ...
    while (1) {
        ...
        qtypes_6af[index_questions_6af] =
            qtype_str_to_code_6af(strupr(qtype_str));
...
int dns6af_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *dst_ip,
    ...
    if (label_type_6af == DNS_LTYPE_RAW || label_type_6af == DNS_LTYPE_STR) {
        // For num_questions_6af == 1, we handle this in per-thread init. Do
        // less work
        if (num_questions_6af > 1) {
            uint16_t payload_len =
        ...
        case DNS_LTYPE_RANDOM: {
            ...
            dns_random_bytes_6af(new_label, 8, charset_alpha_lower_6af, 26,
                                 aes);
            new_label[8] = '\0';
            break;
        }
        ...
        dns_packet_lens_6af[index] = sizeof(dns_header) +
                                     qname_lens_6af[index] +
