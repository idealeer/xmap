...
// xmap boilerplate
probe_module_t module_dns6aecsv;
static int     dns_num_ports_6aecsv;

const char     default_domain_6aecsv[] = "www.qq.com";
const uint16_t default_qtype_6aecsv    = DNS_QTYPE_A;
const char    *dns6aecsv_usage_error =
...
const unsigned char *charset_alpha_lower_6aecsv =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_6aecsv;
static uint16_t *dns_packet_lens_6aecsv; // Not including udp header
static uint16_t *qname_lens_6aecsv;      // domain_len list
static char    **qnames_6aecsv;          // domain list for query
static uint16_t *qtypes_6aecsv;          // query_type list
static char    **domains_6aecsv;         // domain strs
static int       num_questions_6aecsv   = 0;
static int       index_questions_6aecsv = 0;

const char      default_option_qname_6aecsv[]   = {0x00};
static int      default_option_qname_len_6aecsv = 1;
static uint16_t default_option_udpsize_6aecsv   = 4096;
const char      default_option_rdata_6aecsv[];
static int      default_option_rdata_len_6aecsv = 15;

/* Array of qtypes_6aecsv we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_6aecsv (below)
...
int8_t qtype_qtype_to_strid_6aecsv[65536] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_6aecsv() {
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_A]        = 0;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_NS]       = 1;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_CNAME]    = 2;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_SOA]      = 3;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_PTR]      = 4;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_MX]       = 5;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_TXT]      = 6;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_AAAA]     = 7;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_RRSIG]    = 8;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_ALL]      = 9;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_SIG]      = 10;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_SRV]      = 11;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_DS]       = 12;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_DNSKEY]   = 13;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_TLSA]     = 14;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_SVCB]     = 15;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_HTTPS]    = 16;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_CAA]      = 17;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_HTTPSSVC] = 18;
    qtype_qtype_to_strid_6aecsv[DNS_QTYPE_OPT]      = 19;
}

static uint16_t qtype_str_to_code_6aecsv(const char *str) {
    for (int i = 0; i < qtype_strs_len_6aecsv; i++) {
        if (strcmp(qtype_strs_6aecsv[i], str) == 0)
            return qtype_strid_to_qtype_6aecsv[i];
    }

    return 0;
}

static char    *label_6aecsv      = NULL;
static uint16_t label_len_6aecsv  = 0;
static uint16_t label_type_6aecsv = DNS_LTYPE_RAW;
static uint16_t recursive_6aecsv  = 1;

static uint16_t domain_to_qname_6aecsv(char      **qname_handle,
                                       const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_6aecsv(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_6aecsv[i] =
            domain_to_qname_6aecsv(&qnames_6aecsv[i], domains[i]);
        if (domains[i] != (char *) default_domain_6aecsv) {
            free(domains[i]);
        }
        dns_packet_lens_6aecsv[i] =
            sizeof(dns_header) + qname_lens_6aecsv[i] +
            sizeof(dns_question_tail) + default_option_qname_len_6aecsv +
            sizeof(dns_option_tail) + default_option_rdata_len_6aecsv;
        if (dns_packet_lens_6aecsv[i] > DNS_SEND_LEN) {
            log_fatal("dns6aecsv", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_6aecsv[i], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }
...
static uint16_t get_name_helper_6aecsv(const char *data, uint16_t data_len,
                                       const char *payload,
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dns6aecsv", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dns6aecsv", "_get_name_helper, ptr encountered");
            ...
            log_trace("dns6aecsv", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            uint16_t rec_bytes_consumed = get_name_helper_6aecsv(
            ...
            if (byte > name_len) {
                log_warn("dns6aecsv", "Exceeded static name field allocation.");
                return 0;
            }
...
static char *get_name_6aecsv(const char *data, uint16_t data_len,
                             const char *payload, uint16_t payload_len,
                             uint16_t *bytes_consumed) {
    log_trace("dns6aecsv", "call to get_name_6aecsv, data_len: %d", data_len);
    char *name      = xmalloc(MAX_NAME_LENGTH);
    *bytes_consumed = get_name_helper_6aecsv(
    ...
    log_trace(
        "dns6aecsv",
        "return success from get_name_6aecsv, bytes_consumed: %d, string: %s",
        *bytes_consumed, name);

    return name;
}

static bool process_response_question_6aecsv(char **data, uint16_t *data_len,
                                             const char *payload,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name  = get_name_6aecsv(*data, *data_len, payload,
...
static bool process_response_answer_6aecsv(char **data, uint16_t *data_len,
    ...
    if (type > MAX_QTYPE ||
        qtype_qtype_to_strid_6aecsv[type] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(
            afs, "type_str",
            (char *) qtype_strs_6aecsv[qtype_qtype_to_strid_6aecsv[type]], 0);
    }
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        char    *rdata_name = get_name_6aecsv(rdata, rdlength, payload,
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name =
                get_name_6aecsv(rdata + 2, rdlength - 2, payload, payload_len,
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace(
        "dns6aecsv",
        "return success from process_response_answer_6aecsv, data_len: %d",
        *data_len);

    return 0;
}

static int load_question_from_str_6aecsv(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (index_questions_6aecsv >= num_questions_6aecsv) {
            log_error("dns6aecsv", "less probes than questions configured. Add "
        ...
        if (label_type_6aecsv == DNS_LTYPE_STR) {
            domains_6aecsv[index_questions_6aecsv] =
                xmalloc(label_len_6aecsv + 1 + domain_len + 1);
            strncpy(domains_6aecsv[index_questions_6aecsv], label_6aecsv,
                    label_len_6aecsv);
            domains_6aecsv[index_questions_6aecsv][label_len_6aecsv] = '.';
            strncpy(domains_6aecsv[index_questions_6aecsv] + label_len_6aecsv +
                        1,
                    probe_q_delimiter_p + 1, domain_len);
            domains_6aecsv[index_questions_6aecsv]
                          [label_len_6aecsv + 1 + domain_len] = '\0';
        } else {
            domains_6aecsv[index_questions_6aecsv] = xmalloc(domain_len + 1);
            strncpy(domains_6aecsv[index_questions_6aecsv],
                    probe_q_delimiter_p + 1, domain_len);
            domains_6aecsv[index_questions_6aecsv][domain_len] = '\0';
        }
        ...
        qtypes_6aecsv[index_questions_6aecsv] =
            qtype_str_to_code_6aecsv(strupr(qtype_str));
        if (!qtypes_6aecsv[index_questions_6aecsv]) {
            log_error("dns6aecsv", "incorrect qtype supplied: %s", qtype_str);
            free(qtype_str);
            return EXIT_FAILURE;
        }
        free(qtype_str);

        index_questions_6aecsv++;
        if (probe_arg_delimiter_p)
...
static int load_question_from_file_6aecsv(const char *file) {
    log_debug("dns6aecsv", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dns6aecsv", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_6aecsv(line)) return EXIT_FAILURE;
    }

    return EXIT_SUCCESS;
}

int dns_random_bytes_6aecsv(char *dst, int len, const unsigned char *charset,
...
static int dns6aecsv_global_init(struct state_conf *conf) {
    num_questions_6aecsv = conf->target_index_num;

    if (!conf->probe_args) {
        conf->target_index_num = 1;
        num_questions_6aecsv   = 1;
    }

    if (num_questions_6aecsv < 1) {
        log_fatal("dns6aecsv",
    ...
    dns_packets_6aecsv     = xmalloc(sizeof(char *) * num_questions_6aecsv);
    dns_packet_lens_6aecsv = xmalloc(sizeof(uint16_t) * num_questions_6aecsv);
    qname_lens_6aecsv      = xmalloc(sizeof(uint16_t) * num_questions_6aecsv);
    qnames_6aecsv          = xmalloc(sizeof(char *) * num_questions_6aecsv);
    qtypes_6aecsv          = xmalloc(sizeof(uint16_t) * num_questions_6aecsv);
    domains_6aecsv         = xmalloc(sizeof(char *) * num_questions_6aecsv);

    for (int i = 0; i < num_questions_6aecsv; i++) {
        domains_6aecsv[i] = (char *) default_domain_6aecsv;
        qtypes_6aecsv[i]  = default_qtype_6aecsv;
    }

    // This is xmap boilerplate. Why do I have to write this?
    dns_num_ports_6aecsv = conf->source_port_last - conf->source_port_first + 1;
    setup_qtype_str_map_6aecsv();
    ...
    if (conf->probe_args &&
        ...
        if (strncasecmp(conf->probe_args, "raw", 3) == 0) {
            label_type_6aecsv = DNS_LTYPE_RAW;
            log_debug("dns6aecsv", "raw label prefix");
        } else if (strncasecmp(conf->probe_args, "time", 4) == 0) {
            label_type_6aecsv = DNS_LTYPE_TIME;
            log_debug("dns6aecsv", "time label prefix");
        } else if (strncasecmp(conf->probe_args, "random", 6) == 0) {
            label_type_6aecsv = DNS_LTYPE_RANDOM;
            log_debug("dns6aecsv", "random label prefix");
            ...
            label_len_6aecsv = c - conf->probe_args;
            label_6aecsv     = xmalloc(label_len_6aecsv);
            strncpy(label_6aecsv, conf->probe_args, label_len_6aecsv);
            ++c;
            log_debug("dns6aecsv", "label prefix: %s, len: %d", label_6aecsv,
                      label_len_6aecsv);
        } else if (strncasecmp(conf->probe_args, "dst-ip", 6) == 0) {
            label_type_6aecsv = DNS_LTYPE_SRCIP;
            log_debug("dns6aecsv", "dst-ip label prefix");
        } else {
            log_error("dns6aecsv", dns6aecsv_usage_error);
            return EXIT_FAILURE;
        }
...
static int dns6aecsv_global_cleanup(UNUSED struct state_conf *xconf,
                                    UNUSED struct state_send *xsend,
                                    UNUSED struct state_recv *xrecv) {
    if (dns_packets_6aecsv) {
        for (int i = 0; i < num_questions_6aecsv; i++) {
            if (dns_packets_6aecsv[i]) {
                free(dns_packets_6aecsv[i]);
            }
        }
        free(dns_packets_6aecsv);
    }
    dns_packets_6aecsv = NULL;

    if (qnames_6aecsv) {
        for (int i = 0; i < num_questions_6aecsv; i++) {
            if (qnames_6aecsv[i]) {
                free(qnames_6aecsv[i]);
            }
        }
        free(qnames_6aecsv);
    }
    ...
    if (dns_packet_lens_6aecsv) {
        free(dns_packet_lens_6aecsv);
    }

    if (qname_lens_6aecsv) {
        free(qname_lens_6aecsv);
    }
...
int dns6aecsv_thread_init(void *buf, macaddr_t *src, macaddr_t *gw,
                          void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_6aecsv == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth6_header(eth_header, src, gw);

    struct ip6_hdr *ip6_header = (struct ip6_hdr *) (&eth_header[1]);
    uint16_t payload_len = sizeof(struct udphdr) + dns_packet_lens_6aecsv[0];
    make_ip6_header(ip6_header, IPPROTO_UDP, payload_len);

    struct udphdr *udp6_header = (struct udphdr *) (&ip6_header[1]);
    uint16_t       udp_len = sizeof(struct udphdr) + dns_packet_lens_6aecsv[0];
    make_udp_header(udp6_header, udp_len);

    char *payload = (char *) (&udp6_header[1]);
    module_dns6aecsv.packet_length =
        sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
        sizeof(struct udphdr) + dns_packet_lens_6aecsv[0];
    assert(module_dns6aecsv.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_6aecsv[0], dns_packet_lens_6aecsv[0]);
...
int dns6aecsv_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    port_h_t src_port =
        get_src_port(dns_num_ports_6aecsv, probe_num, validation);
    uint16_t dns_txid = get_dnsa_txid(validation, probe_num);

    if (label_type_6aecsv == DNS_LTYPE_RAW ||
        label_type_6aecsv == DNS_LTYPE_STR) {
        // For num_questions_6aecsv == 1, we handle this in per-thread init. Do
        // less work
        if (num_questions_6aecsv > 1) {
            uint16_t payload_len =
                sizeof(struct udphdr) + dns_packet_lens_6aecsv[index];
            make_ip6_header(ip6_header, IPPROTO_UDP, payload_len);
            ...
            *buf_len = sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
            ...
            memcpy(payload, dns_packets_6aecsv[index],
                   dns_packet_lens_6aecsv[index]);
        }
        ...
        case DNS_LTYPE_RANDOM: {
            ...
            dns_random_bytes_6aecsv(new_label, 8, charset_alpha_lower_6aecsv,
                                    26, aes);
            new_label[8] = '\0';
            break;
        }
        ...
        default:
            log_fatal("dns6aecsv", dns6aecsv_usage_error);
            return EXIT_FAILURE;
        }

        snprintf(new_domain, MAX_NAME_LENGTH, "%s-%s", new_label,
                 domains_6aecsv[index]);

        // dns packet
        free(qnames_6aecsv[index]);

        qname_lens_6aecsv[index] =
            domain_to_qname_6aecsv(&qnames_6aecsv[index], new_domain);
        dns_packet_lens_6aecsv[index] =
            sizeof(dns_header) + qname_lens_6aecsv[index] +
            sizeof(dns_question_tail) + default_option_qname_len_6aecsv +
            sizeof(dns_option_tail) + default_option_rdata_len_6aecsv;
        if (dns_packet_lens_6aecsv[index] > DNS_SEND_LEN) {
            log_fatal("dns6aecsv", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_6aecsv[index], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }
...
void dns6aecsv_print_packet(FILE *fp, void *packet) {
    ...
    uint16_t bytes_consumed = 0;
    char *question_name = get_name_6aecsv(data, data_len, (char *) dns_header_p,
    ...
    dns_option_tail *option_tail_p =
        (dns_option_tail *) (option_qname_p + default_option_qname_len_6aecsv);
    ...
    fprintf(fp,
            ...
            question_name,
            qtype_strs_6aecsv
                [qtype_qtype_to_strid_6aecsv[(uint16_t) tail_p->qtype]],
...
int dns6aecsv_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip6_header->ip6_nxt == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_6aecsv, validation)) {
            return PACKET_INVALID;
        }
...
void dns6aecsv_process_packet(const u_char *packet, uint32_t len,
    ...
    if (ip6_header->ip6_nxt == IPPROTO_UDP) {
        ...
        // packet prevent this.
        log_fatal("dns6aecsv", "Die. This can only happen if you "
...
probe_module_t module_dns6aecsv = {
    ...
    .port_args       = 1,
    .global_init     = &dns6aecsv_global_init,
    .thread_init     = &dns6aecsv_thread_init,
    .make_packet     = &dns6aecsv_make_packet,
    .print_packet    = &dns6aecsv_print_packet,
    .validate_packet = &dns6aecsv_validate_packet,
    .process_packet  = &dns6aecsv_process_packet,
    .close           = &dns6aecsv_global_cleanup,
    ...
    .helptext =
        ...
        "Besides, ECS with fixed src_ipv6/56/0 is appended to the tail.\n"
