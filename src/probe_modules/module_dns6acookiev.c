 ...
 *
 * Usage: xmap -p 53 --probe-module=dns6acookiev
 *--probe-args="raw:text:A,qq.com" -O json --output-fields=* 8.8.8.8
 *
...
// xmap boilerplate
probe_module_t module_dns6acookiev;
static int     dns_num_ports_6acookiev;

const char     default_domain_6acookiev[] = "www.qq.com";
const uint16_t default_qtype_6acookiev    = DNS_QTYPE_A;
const char    *dns6acookiev_usage_error =
...
const unsigned char *charset_alpha_lower_6acookiev =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_6acookiev;
static uint16_t *dns_packet_lens_6acookiev; // Not including udp header
static uint16_t *qname_lens_6acookiev;      // domain_len list
static char    **qnames_6acookiev;          // domain list for query
static uint16_t *qtypes_6acookiev;          // query_type list
static char    **domains_6acookiev;         // domain strs
static int       num_questions_6acookiev   = 0;
static int       index_questions_6acookiev = 0;

const char      default_option_qname_6acookiev[]   = {0x00};
static int      default_option_qname_len_6acookiev = 1;
static uint16_t default_option_udpsize_6acookiev   = 4096;
const char      default_option_rdata_6acookiev[];
static int      default_option_rdata_len_6acookiev = 20;

/* Array of qtypes_6acookiev we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_6acookiev (below)
 * qtype_qtype_to_strid_6acookiev (below, and setup_qtype_str_map_6acookiev())
 */
const char *qtype_strs_6acookiev[] = {
    "A",    "NS",    "CNAME", "SOA", "PTR",      "MX", "TXT",
    "AAAA", "RRSIG", "ANY",   "SIG", "SRV",      "DS", "DNSKEY",
    "TLSA", "SVCB",  "HTTPS", "CAA", "HTTPSSVC", "OPT"};
const int qtype_strs_len_6acookiev = 20;

const dns_qtype qtype_strid_to_qtype_6acookiev[] = {
...
int8_t qtype_qtype_to_strid_6acookiev[65536] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_6acookiev() {
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_A]        = 0;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_NS]       = 1;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_CNAME]    = 2;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_SOA]      = 3;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_PTR]      = 4;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_MX]       = 5;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_TXT]      = 6;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_AAAA]     = 7;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_RRSIG]    = 8;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_ALL]      = 9;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_SIG]      = 10;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_SRV]      = 11;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_DS]       = 12;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_DNSKEY]   = 13;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_TLSA]     = 14;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_SVCB]     = 15;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_HTTPS]    = 16;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_CAA]      = 17;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_HTTPSSVC] = 18;
    qtype_qtype_to_strid_6acookiev[DNS_QTYPE_OPT]      = 19;
}

static uint16_t qtype_str_to_code_6acookiev(const char *str) {
    for (int i = 0; i < qtype_strs_len_6acookiev; i++) {
        if (strcmp(qtype_strs_6acookiev[i], str) == 0)
            return qtype_strid_to_qtype_6acookiev[i];
    }
...
static int build_global_dns_packets_6acookiev(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        ...
        dns_packets_6acookiev[i] = xmalloc(dns_packet_lens_6acookiev[i]);
        dns_header *dns_header_p = (dns_header *) dns_packets_6acookiev[i];
        char       *qname_p = dns_packets_6acookiev[i] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_6acookiev[i] +
                                   sizeof(dns_header) +
                                   qname_lens_6acookiev[i]);
        char *option_qname_p =
            (char *) (dns_packets_6acookiev[i] + sizeof(dns_header) +
                      qname_lens_6acookiev[i] + sizeof(dns_question_tail));
        dns_option_tail *option_tail_p =
            (dns_option_tail *) (dns_packets_6acookiev[i] + sizeof(dns_header) +
                                 qname_lens_6acookiev[i] +
                                 sizeof(dns_question_tail) +
                                 default_option_qname_len_6acookiev);
        dns_option_cookie *option_cookie_p =
            (dns_option_cookie *) (dns_packets_6acookiev[i] +
                                   sizeof(dns_header) +
                                   qname_lens_6acookiev[i] +
                                   sizeof(dns_question_tail) +
                                   default_option_qname_len_6acookiev +
        ...
        memcpy(qname_p, qnames_6acookiev[i], qname_lens_6acookiev[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_6acookiev[i]);
...
static uint16_t get_name_helper_6acookiev(const char *data, uint16_t data_len,
                                          const char *payload,
                                          uint16_t payload_len, char *name,
                                          uint16_t name_len,
                                          uint16_t recursion_level) {
    log_trace("dns6acookiev",
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dns6acookiev", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dns6acookiev", "_get_name_helper, ptr encountered");
            ...
            log_trace("dns6acookiev", "_get_name_helper. ptr offset 0x%x",
                      offset);
            ...
            uint16_t rec_bytes_consumed = get_name_helper_6acookiev(
...
static char *get_name_6acookiev(const char *data, uint16_t data_len,
                                const char *payload, uint16_t payload_len,
                                uint16_t *bytes_consumed) {
    log_trace("dns6acookiev", "call to get_name_6acookiev, data_len: %d",
              data_len);
    char *name      = xmalloc(MAX_NAME_LENGTH);
    *bytes_consumed = get_name_helper_6acookiev(
    ...
    log_trace("dns6acookiev",
              "return success from get_name_6acookiev, bytes_consumed: %d, "
              "string: %s",
              *bytes_consumed, name);
...
static bool process_response_question_6acookiev(char **data, uint16_t *data_len,
                                                const char *payload,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name  = get_name_6acookiev(*data, *data_len, payload,
    ...
    if (qtype > MAX_QTYPE ||
        qtype_qtype_to_strid_6acookiev[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(
            qfs, "qtype_str",
            (char *)
                qtype_strs_6acookiev[qtype_qtype_to_strid_6acookiev[qtype]],
            0);
    }
...
static bool process_response_answer_6acookiev(char **data, uint16_t *data_len,
                                              const char *payload,
                                              uint16_t    payload_len,
                                              fieldset_t *list) {
    log_trace("dns6acookiev",
              "call to process_response_answer_6acookiev, data_len: %d",
              *data_len);
    // Payload is the start of the DNS packet, including header
    // data is handle to the start of this RR
    // data_len is a pointer to the how much total data we have to work
    // with. This is awful. I'm bad and should feel bad.
    uint16_t bytes_consumed = 0;
    char    *answer_name    = get_name_6acookiev(*data, *data_len, payload,
                                                 payload_len, &bytes_consumed);
    // Error.
    if (answer_name == NULL) {
        return 1;
    ...
    if (type > MAX_QTYPE ||
        qtype_qtype_to_strid_6acookiev[type] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(
            afs, "type_str",
            (char *) qtype_strs_6acookiev[qtype_qtype_to_strid_6acookiev[type]],
            0);
    }
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        char    *rdata_name           = get_name_6acookiev(
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name =
                get_name_6acookiev(rdata + 2, rdlength - 2, payload,
        ...
        if (option_dlength >= sizeof(dns_option_cookie)) {
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace(
        "dns6acookiev",
        "return success from process_response_answer_6acookiev, data_len: %d",
        *data_len);
...
static int dns6acookiev_global_cleanup(UNUSED struct state_conf *xconf,
                                       UNUSED struct state_send *xsend,
                                       UNUSED struct state_recv *xrecv) {
    if (dns_packets_6acookiev) {
        for (int i = 0; i < num_questions_6acookiev; i++) {
            if (dns_packets_6acookiev[i]) {
                free(dns_packets_6acookiev[i]);
            }
        }
        free(dns_packets_6acookiev);
    }
    dns_packets_6acookiev = NULL;

    if (qnames_6acookiev) {
        for (int i = 0; i < num_questions_6acookiev; i++) {
            if (qnames_6acookiev[i]) {
                free(qnames_6acookiev[i]);
            }
        }
        free(qnames_6acookiev);
    }
    qnames_6acookiev = NULL;

    if (dns_packet_lens_6acookiev) {
        free(dns_packet_lens_6acookiev);
    }

    if (qname_lens_6acookiev) {
        free(qname_lens_6acookiev);
    }

    if (qtypes_6acookiev) {
        free(qtypes_6acookiev);
    }

    free(label_6acookiev);

    return EXIT_SUCCESS;
}

int dns6acookiev_thread_init(void *buf, macaddr_t *src, macaddr_t *gw,
                             void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_6acookiev == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth6_header(eth_header, src, gw);

    struct ip6_hdr *ip6_header = (struct ip6_hdr *) (&eth_header[1]);
    uint16_t payload_len = sizeof(struct udphdr) + dns_packet_lens_6acookiev[0];
    make_ip6_header(ip6_header, IPPROTO_UDP, payload_len);

    struct udphdr *udp6_header = (struct udphdr *) (&ip6_header[1]);
    uint16_t udp_len = sizeof(struct udphdr) + dns_packet_lens_6acookiev[0];
    make_udp_header(udp6_header, udp_len);

    char *payload = (char *) (&udp6_header[1]);
    module_dns6acookiev.packet_length =
        sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
        sizeof(struct udphdr) + dns_packet_lens_6acookiev[0];
    assert(module_dns6acookiev.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_6acookiev[0], dns_packet_lens_6acookiev[0]);
...
int dns6acookiev_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    port_h_t src_port =
        get_src_port(dns_num_ports_6acookiev, probe_num, validation);
    uint16_t dns_txid = get_dnsa_txid(validation, probe_num);

    if (label_type_6acookiev == DNS_LTYPE_RAW ||
        label_type_6acookiev == DNS_LTYPE_STR) {
        // For num_questions_6acookiev == 1, we handle this in per-thread init.
        // Do less work
        if (num_questions_6acookiev > 1) {
            uint16_t payload_len =
                sizeof(struct udphdr) + dns_packet_lens_6acookiev[index];
            make_ip6_header(ip6_header, IPPROTO_UDP, payload_len);
            ...
            *buf_len = sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
            ...
            memcpy(payload, dns_packets_6acookiev[index],
                   dns_packet_lens_6acookiev[index]);
        }
        ...
        case DNS_LTYPE_RANDOM: {
            ...
            dns_random_bytes_6acookiev(new_label, 8,
                                       charset_alpha_lower_6acookiev, 26, aes);
            new_label[8] = '\0';
            break;
        }
        ...
        default:
            log_fatal("dns6acookiev", dns6acookiev_usage_error);
            return EXIT_FAILURE;
        }

        snprintf(new_domain, MAX_NAME_LENGTH, "%s-%s", new_label,
                 domains_6acookiev[index]);

        // dns packet
        free(qnames_6acookiev[index]);

        qname_lens_6acookiev[index] =
            domain_to_qname_6acookiev(&qnames_6acookiev[index], new_domain);
        dns_packet_lens_6acookiev[index] =
            sizeof(dns_header) + qname_lens_6acookiev[index] +
            sizeof(dns_question_tail) + default_option_qname_len_6acookiev +
            sizeof(dns_option_tail) + default_option_rdata_len_6acookiev;
        if (dns_packet_lens_6acookiev[index] > DNS_SEND_LEN) {
            log_fatal("dns6acookiev",
                      "DNS packet bigger (%d) than our limit (%d)",
...
void dns6acookiev_print_packet(FILE *fp, void *packet) {
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name  = get_name_6acookiev(
    ...
    fprintf(fp,
            ...
            question_name,
            qtype_strs_6acookiev
                [qtype_qtype_to_strid_6acookiev[(uint16_t) tail_p->qtype]],
...
int dns6acookiev_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
void dns6acookiev_process_packet(const u_char *packet, uint32_t len,
    ...
    if (ip6_header->ip6_nxt == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_6acookiev(
        ...
        for (int i = 0; i < ntohs(dns_header_p->ancount) && !err; i++) {
            err = process_response_answer_6acookiev(
                &data, &data_len, (char *) dns_header_p, udp_len, list);
        }
        ...
        // packet prevent this.
        log_fatal("dns6acookiev", "Die. This can only happen if you "
...
probe_module_t module_dns6acookiev = {
    ...
    .port_args       = 1,
    .global_init     = &dns6acookiev_global_init,
    .thread_init     = &dns6acookiev_thread_init,
    .make_packet     = &dns6acookiev_make_packet,
    .print_packet    = &dns6acookiev_print_packet,
    .validate_packet = &dns6acookiev_validate_packet,
    .process_packet  = &dns6acookiev_process_packet,
    .close           = &dns6acookiev_global_cleanup,
