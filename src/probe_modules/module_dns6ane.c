...
static int build_global_dns_packets_6ane(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        ...
        dns_packet_lens_6ane[i] =
            sizeof(dns_header) + qname_lens_6ane[i] +
...
static bool process_response_question_6ane(char **data, uint16_t *data_len,
    ...
    if (qtype > MAX_QTYPE ||
        qtype_qtype_to_strid_6ane[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(
            qfs, "qtype_str",
            (char *) qtype_strs_6ane[qtype_qtype_to_strid_6ane[qtype]], 0);
    }
...
static bool process_response_answer_6ane(char **data, uint16_t *data_len,
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name =
                get_name_6ane(rdata + 2, rdlength - 2, payload, payload_len,
                              &rdata_bytes_consumed);
            if (rdata_name == NULL) {
                fs_add_uint64(afs, "rdata_is_parsed", 0);
                fs_add_binary(afs, "rdata", rdlength, rdata, 0);
...
static int load_question_from_str_6ane(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (label_type_6ane == DNS_LTYPE_STR) {
            ...
            strncpy(domains_6ane[index_questions_6ane], label_6ane,
                    label_len_6ane);
    ...
    if (label_type_6ane == DNS_LTYPE_RAW || label_type_6ane == DNS_LTYPE_STR) {
        ...
        if (num_questions_6ane > 1) {
            ...
            *buf_len = sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
            ...
            memcpy(payload, dns_packets_6ane[index],
        ...
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_6ane[index] +
                                   sizeof(dns_header) + qname_lens_6ane[index]);
        char *option_qname_p =
