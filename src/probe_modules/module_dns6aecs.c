 ...
 *
 * Usage: xmap -p 53 --probe-module=dns6aecs --probe-args="raw:text:A,qq.com"
...
// xmap boilerplate
probe_module_t module_dns6aecs;
static int     dns_num_ports_6aecs;

const char     default_domain_6aecs[] = "www.qq.com";
const uint16_t default_qtype_6aecs    = DNS_QTYPE_A;
const char    *dns6aecs_usage_error =
...
const unsigned char *charset_alpha_lower_6aecs =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_6aecs;
static uint16_t *dns_packet_lens_6aecs; // Not including udp header
static uint16_t *qname_lens_6aecs;      // domain_len list
static char    **qnames_6aecs;          // domain list for query
static uint16_t *qtypes_6aecs;          // query_type list
static char    **domains_6aecs;         // domain strs
static int       num_questions_6aecs   = 0;
static int       index_questions_6aecs = 0;

const char      default_option_qname_6aecs[]   = {0x00};
static int      default_option_qname_len_6aecs = 1;
static uint16_t default_option_udpsize_6aecs   = 4096;
const char      default_option_rdata_6aecs[];
static int      default_option_rdata_len_6aecs = 15;

/* Array of qtypes_6aecs we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_6aecs (below)
 * qtype_qtype_to_strid_6aecs (below, and setup_qtype_str_map_6aecs())
 */
const char *qtype_strs_6aecs[]   = {"A",    "NS",    "CNAME", "SOA",      "PTR",
                                    "MX",   "TXT",   "AAAA",  "RRSIG",    "ANY",
                                    "SIG",  "SRV",   "DS",    "DNSKEY",   "TLSA",
                                    "SVCB", "HTTPS", "CAA",   "HTTPSSVC", "OPT"};
const int   qtype_strs_len_6aecs = 20;

const dns_qtype qtype_strid_to_qtype_6aecs[] = {
...
int8_t qtype_qtype_to_strid_6aecs[65536] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_6aecs() {
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_A]        = 0;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_NS]       = 1;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_CNAME]    = 2;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_SOA]      = 3;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_PTR]      = 4;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_MX]       = 5;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_TXT]      = 6;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_AAAA]     = 7;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_RRSIG]    = 8;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_ALL]      = 9;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_SIG]      = 10;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_SRV]      = 11;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_DS]       = 12;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_DNSKEY]   = 13;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_TLSA]     = 14;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_SVCB]     = 15;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_HTTPS]    = 16;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_CAA]      = 17;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_HTTPSSVC] = 18;
    qtype_qtype_to_strid_6aecs[DNS_QTYPE_OPT]      = 19;
}

static uint16_t qtype_str_to_code_6aecs(const char *str) {
    for (int i = 0; i < qtype_strs_len_6aecs; i++) {
        if (strcmp(qtype_strs_6aecs[i], str) == 0)
            return qtype_strid_to_qtype_6aecs[i];
    }

    return 0;
}

static char    *label_6aecs      = NULL;
static uint16_t label_len_6aecs  = 0;
static uint16_t label_type_6aecs = DNS_LTYPE_RAW;
static uint16_t recursive_6aecs  = 1;

static uint16_t domain_to_qname_6aecs(char **qname_handle, const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_6aecs(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_6aecs[i] =
            domain_to_qname_6aecs(&qnames_6aecs[i], domains[i]);
        if (domains[i] != (char *) default_domain_6aecs) {
            free(domains[i]);
        }
        dns_packet_lens_6aecs[i] =
            sizeof(dns_header) + qname_lens_6aecs[i] +
            sizeof(dns_question_tail) + default_option_qname_len_6aecs +
            sizeof(dns_option_tail) + default_option_rdata_len_6aecs;
        if (dns_packet_lens_6aecs[i] > DNS_SEND_LEN) {
            log_fatal("dns6aecs", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_6aecs[i], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }

        dns_packets_6aecs[i]            = xmalloc(dns_packet_lens_6aecs[i]);
        dns_header        *dns_header_p = (dns_header *) dns_packets_6aecs[i];
        char              *qname_p = dns_packets_6aecs[i] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_6aecs[i] + sizeof(dns_header) +
                                   qname_lens_6aecs[i]);
        char *option_qname_p =
            (char *) (dns_packets_6aecs[i] + sizeof(dns_header) +
                      qname_lens_6aecs[i] + sizeof(dns_question_tail));
        dns_option_tail *option_tail_p =
            (dns_option_tail *) (dns_packets_6aecs[i] + sizeof(dns_header) +
                                 qname_lens_6aecs[i] +
                                 sizeof(dns_question_tail) +
                                 default_option_qname_len_6aecs);
        dns_option_ecs *option_ecs_p =
            (dns_option_ecs *) (dns_packets_6aecs[i] + sizeof(dns_header) +
                                qname_lens_6aecs[i] +
                                sizeof(dns_question_tail) +
                                default_option_qname_len_6aecs +
        ...
        dns_header_p->rd = recursive_6aecs; // Is one bit. Don't need htons
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_6aecs[i], qname_lens_6aecs[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_6aecs[i]);
        ...
        memcpy(option_qname_p, default_option_qname_6aecs,
               default_option_qname_len_6aecs);
        option_tail_p->type    = htons(DNS_QTYPE_OPT);
        option_tail_p->udpsize = htons(default_option_udpsize_6aecs);
        option_tail_p->dlength = htons(default_option_rdata_len_6aecs);
...
static uint16_t get_name_helper_6aecs(const char *data, uint16_t data_len,
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dns6aecs", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dns6aecs", "_get_name_helper, ptr encountered");
            ...
            log_trace("dns6aecs", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            uint16_t rec_bytes_consumed = get_name_helper_6aecs(
            ...
            log_trace("dns6aecs", "_get_name_helper, segment 0x%hx encountered",
                      byte);
...
static char *get_name_6aecs(const char *data, uint16_t data_len,
                            const char *payload, uint16_t payload_len,
                            uint16_t *bytes_consumed) {
    log_trace("dns6aecs", "call to get_name_6aecs, data_len: %d", data_len);
    char *name      = xmalloc(MAX_NAME_LENGTH);
    *bytes_consumed = get_name_helper_6aecs(
    ...
    log_trace(
        "dns6aecs",
        "return success from get_name_6aecs, bytes_consumed: %d, string: %s",
        *bytes_consumed, name);

    return name;
}

static bool process_response_question_6aecs(char **data, uint16_t *data_len,
                                            const char *payload,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name =
        get_name_6aecs(*data, *data_len, payload, payload_len, &bytes_consumed);
    // Error.
    ...
    if (qtype > MAX_QTYPE ||
        qtype_qtype_to_strid_6aecs[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(
            qfs, "qtype_str",
            (char *) qtype_strs_6aecs[qtype_qtype_to_strid_6aecs[qtype]], 0);
    }
...
static bool process_response_answer_6aecs(char **data, uint16_t *data_len,
                                          const char *payload,
                                          uint16_t    payload_len,
                                          fieldset_t *list) {
    log_trace("dns6aecs", "call to process_response_answer_6aecs, data_len: %d",
              *data_len);
    // Payload is the start of the DNS packet, including header
    // data is handle to the start of this RR
    // data_len is a pointer to the how much total data we have to work
    ...
    if (type > MAX_QTYPE || qtype_qtype_to_strid_6aecs[type] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(
            afs, "type_str",
            (char *) qtype_strs_6aecs[qtype_qtype_to_strid_6aecs[type]], 0);
    }
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        char *rdata_name = get_name_6aecs(rdata, rdlength, payload, payload_len,
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name =
                get_name_6aecs(rdata + 2, rdlength - 2, payload, payload_len,
                               &rdata_bytes_consumed);
            if (rdata_name == NULL) {
                fs_add_uint64(afs, "rdata_is_parsed", 0);
                fs_add_binary(afs, "rdata", rdlength, rdata, 0);
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace("dns6aecs",
              "return success from process_response_answer_6aecs, data_len: %d",
              *data_len);

    return 0;
}

static int load_question_from_str_6aecs(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (index_questions_6aecs >= num_questions_6aecs) {
            log_error("dns6aecs", "less probes than questions configured. Add "
        ...
        if (label_type_6aecs == DNS_LTYPE_STR) {
            domains_6aecs[index_questions_6aecs] =
                xmalloc(label_len_6aecs + 1 + domain_len + 1);
            strncpy(domains_6aecs[index_questions_6aecs], label_6aecs,
                    label_len_6aecs);
            domains_6aecs[index_questions_6aecs][label_len_6aecs] = '.';
            strncpy(domains_6aecs[index_questions_6aecs] + label_len_6aecs + 1,
                    probe_q_delimiter_p + 1, domain_len);
            domains_6aecs[index_questions_6aecs]
                         [label_len_6aecs + 1 + domain_len] = '\0';
        } else {
            domains_6aecs[index_questions_6aecs] = xmalloc(domain_len + 1);
            strncpy(domains_6aecs[index_questions_6aecs],
                    probe_q_delimiter_p + 1, domain_len);
            domains_6aecs[index_questions_6aecs][domain_len] = '\0';
        }
        ...
        qtypes_6aecs[index_questions_6aecs] =
            qtype_str_to_code_6aecs(strupr(qtype_str));
        if (!qtypes_6aecs[index_questions_6aecs]) {
            log_error("dns6aecs", "incorrect qtype supplied: %s", qtype_str);
            free(qtype_str);
            return EXIT_FAILURE;
        }
        free(qtype_str);

        index_questions_6aecs++;
        if (probe_arg_delimiter_p)
...
static int load_question_from_file_6aecs(const char *file) {
    log_debug("dns6aecs", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dns6aecs", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_6aecs(line)) return EXIT_FAILURE;
    }

    return EXIT_SUCCESS;
}

int dns_random_bytes_6aecs(char *dst, int len, const unsigned char *charset,
...
static int dns6aecs_global_init(struct state_conf *conf) {
    num_questions_6aecs = conf->target_index_num;

    if (!conf->probe_args) {
        conf->target_index_num = 1;
        num_questions_6aecs    = 1;
    }

    if (num_questions_6aecs < 1) {
        log_fatal("dns6aecs", "invalid number of probes for the DNS module: %d",
                  num_questions_6aecs);
    }
...
static int dns6aecs_global_cleanup(UNUSED struct state_conf *xconf,
                                   UNUSED struct state_send *xsend,
                                   UNUSED struct state_recv *xrecv) {
    if (dns_packets_6aecs) {
        for (int i = 0; i < num_questions_6aecs; i++) {
            if (dns_packets_6aecs[i]) {
                free(dns_packets_6aecs[i]);
            }
        }
        free(dns_packets_6aecs);
    }
    dns_packets_6aecs = NULL;
    ...
    if (dns_packet_lens_6aecs) {
        free(dns_packet_lens_6aecs);
    }

    if (qname_lens_6aecs) {
        free(qname_lens_6aecs);
    }
...
int dns6aecs_thread_init(void *buf, macaddr_t *src, macaddr_t *gw,
                         void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_6aecs == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth6_header(eth_header, src, gw);

    struct ip6_hdr *ip6_header = (struct ip6_hdr *) (&eth_header[1]);
    uint16_t payload_len = sizeof(struct udphdr) + dns_packet_lens_6aecs[0];
    make_ip6_header(ip6_header, IPPROTO_UDP, payload_len);

    struct udphdr *udp6_header = (struct udphdr *) (&ip6_header[1]);
    uint16_t       udp_len = sizeof(struct udphdr) + dns_packet_lens_6aecs[0];
    make_udp_header(udp6_header, udp_len);

    char *payload = (char *) (&udp6_header[1]);
    module_dns6aecs.packet_length =
        sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
        sizeof(struct udphdr) + dns_packet_lens_6aecs[0];
    assert(module_dns6aecs.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_6aecs[0], dns_packet_lens_6aecs[0]);
...
int dns6aecs_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    if (label_type_6aecs == DNS_LTYPE_RAW ||
        ...
        dns_packets_6aecs[index] = xmalloc(dns_packet_lens_6aecs[index]);
        dns_header *dns_header_p = (dns_header *) dns_packets_6aecs[index];
        char       *qname_p = dns_packets_6aecs[index] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_6aecs[index] +
                                   sizeof(dns_header) +
                                   qname_lens_6aecs[index]);
        char *option_qname_p =
            (char *) (dns_packets_6aecs[index] + sizeof(dns_header) +
                      qname_lens_6aecs[index] + sizeof(dns_question_tail));
        dns_option_tail *option_tail_p =
            (dns_option_tail *) (dns_packets_6aecs[index] + sizeof(dns_header) +
                                 qname_lens_6aecs[index] +
                                 sizeof(dns_question_tail) +
                                 default_option_qname_len_6aecs);
        dns_option_ecs *option_ecs_p =
            (dns_option_ecs *) (dns_packets_6aecs[index] + sizeof(dns_header) +
                                qname_lens_6aecs[index] +
                                sizeof(dns_question_tail) +
                                default_option_qname_len_6aecs +
                                sizeof(dns_option_tail));
        ...
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_6aecs[index], qname_lens_6aecs[index]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_6aecs[index]);
        // Set the qclass to The Internet (TM) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x01);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
        ...
        uint16_t udp_len = sizeof(struct udphdr) + dns_packet_lens_6aecs[index];
        make_udp_header(udp_header, udp_len);
        ...
        *buf_len      = sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
        ...
        memcpy(payload, dns_packets_6aecs[index], dns_packet_lens_6aecs[index]);
...
void dns6aecs_print_packet(FILE *fp, void *packet) {
    ...
    uint16_t bytes_consumed = 0;
    char *question_name = get_name_6aecs(data, data_len, (char *) dns_header_p,
    ...
    dns_option_tail *option_tail_p =
        (dns_option_tail *) (option_qname_p + default_option_qname_len_6aecs);
    ...
    fprintf(
        ...
        question_name,
        qtype_strs_6aecs[qtype_qtype_to_strid_6aecs[(uint16_t) tail_p->qtype]],
...
int dns6aecs_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip6_header->ip6_nxt == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_6aecs, validation)) {
            return PACKET_INVALID;
        }
...
void dns6aecs_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
...
probe_module_t module_dns6aecs = {
    ...
    .port_args       = 1,
    .global_init     = &dns6aecs_global_init,
    .thread_init     = &dns6aecs_thread_init,
    .make_packet     = &dns6aecs_make_packet,
    .print_packet    = &dns6aecs_print_packet,
    .validate_packet = &dns6aecs_validate_packet,
    .process_packet  = &dns6aecs_process_packet,
    .close           = &dns6aecs_global_cleanup,
