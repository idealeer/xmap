 ...
 * Once a successful dns response is received, a duplicate query will be
 * repeated.
 *
 ...
 *
 * Usage: xmap -p 53 --probe-module=dnsz --probe-args="raw:text:A,qq.com"
...
// xmap boilerplate
probe_module_t module_dnsz;
static int     dns_num_ports_z;

const char     default_domain_z[] = "www.qq.com";
const uint16_t default_qtype_z    = DNS_QTYPE_A;
const char    *dnsz_usage_error   = "unknown DNS probe specification (expected "
...
const unsigned char *charset_alpha_lower_z =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_z;
static uint16_t *dns_packet_lens_z; // Not including udp header
static uint16_t *qname_lens_z;      // domain_len list
static char    **qnames_z;          // domain list for query
static uint16_t *qtypes_z;          // query_type list
static char    **domains_z;         // domain strs
static int       num_questions_z   = 0;
static int       index_questions_z = 0;

/* Array of qtypes_z we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_z (below) qtype_qtype_to_strid_z
...
int8_t qtype_qtype_to_strid_z[256] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_z() {
    qtype_qtype_to_strid_z[DNS_QTYPE_A]     = 0;
    qtype_qtype_to_strid_z[DNS_QTYPE_NS]    = 1;
    qtype_qtype_to_strid_z[DNS_QTYPE_CNAME] = 2;
    qtype_qtype_to_strid_z[DNS_QTYPE_SOA]   = 3;
    qtype_qtype_to_strid_z[DNS_QTYPE_PTR]   = 4;
    qtype_qtype_to_strid_z[DNS_QTYPE_MX]    = 5;
    qtype_qtype_to_strid_z[DNS_QTYPE_TXT]   = 6;
    qtype_qtype_to_strid_z[DNS_QTYPE_AAAA]  = 7;
    qtype_qtype_to_strid_z[DNS_QTYPE_RRSIG] = 8;
    qtype_qtype_to_strid_z[DNS_QTYPE_ALL]   = 9;
}

static uint16_t qtype_str_to_code_z(const char *str) {
    for (int i = 0; i < qtype_strs_len_z; i++) {
        if (strcmp(qtype_strs_z[i], str) == 0) return qtype_strid_to_qtype_z[i];
    }

    return 0;
}

static char    *label_z      = NULL;
static uint16_t label_len_z  = 0;
static uint16_t label_type_z = DNS_LTYPE_RAW;
static uint16_t recursive_z  = 1;

static uint16_t domain_to_qname_z(char **qname_handle, const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_z(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_z[i] = domain_to_qname_z(&qnames_z[i], domains[i]);
        if (domains[i] != (char *) default_domain_z) {
            free(domains[i]);
        }
        dns_packet_lens_z[i] =
            sizeof(dns_header) + qname_lens_z[i] + sizeof(dns_question_tail);
        if (dns_packet_lens_z[i] > DNS_SEND_LEN) {
            log_fatal("dnsz", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_z[i], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }

        dns_packets_z[i]                = xmalloc(dns_packet_lens_z[i]);
        dns_header        *dns_header_p = (dns_header *) dns_packets_z[i];
        char              *qname_p      = dns_packets_z[i] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_z[i] + sizeof(dns_header) +
                                   qname_lens_z[i]);

        // All other header fields should be 0. Except id, which we set
        // per thread. Please recurse as needed.
        dns_header_p->rd = recursive_z; // Is one bit. Don't need htons
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_z[i], qname_lens_z[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_z[i]);
...
static uint16_t get_name_helper_z(const char *data, uint16_t data_len,
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dnsz", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dnsz", "_get_name_helper, ptr encountered");
            // Do we have enough bytes to check ahead?
            if (data_len < 2) {
                log_trace("dnsz", "_get_name_helper OUT. ptr byte encountered. "
                                  "No offset. ERR.");
                return 0;
            }
            ...
            log_trace("dnsz", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            if (recursion_level > 0 || bytes_consumed > 0) {
                ...
                if (name_len < 1) {
                    log_warn("dnsz", "Exceeded static name field allocation.");
                    return 0;
                }
            ...
            uint16_t rec_bytes_consumed = get_name_helper_z(
            ...
            if (rec_bytes_consumed == 0) {
                log_trace("dnsz", "_get_name_helper OUT. rec level %d failed",
                          recursion_level);
                return 0;
            } else {
                bytes_consumed += 2;
            ...
            log_trace("dnsz", "_get_name_helper, segment 0x%hx encountered",
                      byte);
            ...
            // null byte too)?
            if ((byte + 1) > data_len) {
                log_trace("dnsz", "_get_name_helper OUT. ERR. Not enough data "
                                  "for segment %hd");
                return 0;
            }
...
static char *get_name_z(const char *data, uint16_t data_len,
                        const char *payload, uint16_t payload_len,
                        uint16_t *bytes_consumed) {
    log_trace("dnsz", "call to get_name_z, data_len: %d", data_len);
    char *name      = xmalloc(MAX_NAME_LENGTH);
    *bytes_consumed = get_name_helper_z(data, data_len, payload, payload_len,
    ...
    log_trace("dnsz",
              "return success from get_name_z, bytes_consumed: %d, string: %s",
              *bytes_consumed, name);

    return name;
}

static bool process_response_question_z(char **data, uint16_t *data_len,
                                        const char *payload,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name =
        get_name_z(*data, *data_len, payload, payload_len, &bytes_consumed);
    // Error.
    ...
    if (qtype > MAX_QTYPE || qtype_qtype_to_strid_z[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(qfs, "qtype_str",
                      (char *) qtype_strs_z[qtype_qtype_to_strid_z[qtype]], 0);
    }
...
static bool process_response_answer_z(char **data, uint16_t *data_len,
                                      const char *payload, uint16_t payload_len,
                                      fieldset_t *list) {
    log_trace("dnsz", "call to process_response_answer_z, data_len: %d",
              *data_len);
    // Payload is the start of the DNS packet, including header
    // data is handle to the start of this RR
    // data_len is a pointer to the how much total data we have to work
    ...
    if (type > MAX_QTYPE || qtype_qtype_to_strid_z[type] == BAD_QTYPE_VAL) {
        fs_add_string(afs, "type_str", (char *) BAD_QTYPE_STR, 0);
    } else {
        // I've written worse things than this 3rd arg. But I want to be
        // fast.
        fs_add_string(afs, "type_str",
                      (char *) qtype_strs_z[qtype_qtype_to_strid_z[type]], 0);
    }
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        char    *rdata_name = get_name_z(rdata, rdlength, payload, payload_len,
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name = get_name_z(rdata + 2, rdlength - 2, payload,
        ...
        if (rdlength >= 1 && (rdlength - 1) != *(uint8_t *) rdata) {
            log_warn("dnsz", "TXT record with wrong TXT len. Not processing.");
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else if (rdlength < 1) {
            fs_add_uint64(afs, "rdata_is_parsed", 0);
        ...
        if (rdlength != 4) {
            log_warn("dnsz", "A record with IP of length %d. Not processing.",
                     rdlength);
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else {
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace("dnsz",
              "return success from process_response_answer_z, data_len: %d",
              *data_len);

    return 0;
}

static int load_question_from_str_z(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (probe_q_delimiter_p == type_q_str ||
            ...
            log_error("dnsz", dnsz_usage_error);
            return EXIT_FAILURE;
        }

        if (index_questions_z >= num_questions_z) {
            log_error("dnsz", "less probes than questions configured. Add "
        ...
        if (label_type_z == DNS_LTYPE_STR) {
            domains_z[index_questions_z] =
                xmalloc(label_len_z + 1 + domain_len + 1);
            strncpy(domains_z[index_questions_z], label_z, label_len_z);
            domains_z[index_questions_z][label_len_z] = '.';
            strncpy(domains_z[index_questions_z] + label_len_z + 1,
                    probe_q_delimiter_p + 1, domain_len);
            domains_z[index_questions_z][label_len_z + 1 + domain_len] = '\0';
        } else {
            domains_z[index_questions_z] = xmalloc(domain_len + 1);
            strncpy(domains_z[index_questions_z], probe_q_delimiter_p + 1,
                    domain_len);
            domains_z[index_questions_z][domain_len] = '\0';
        }
        ...
        qtypes_z[index_questions_z] = qtype_str_to_code_z(strupr(qtype_str));
        if (!qtypes_z[index_questions_z]) {
            log_error("dnsz", "incorrect qtype supplied. %s", qtype_str);
            free(qtype_str);
            return EXIT_FAILURE;
        }
        free(qtype_str);
...
static int load_question_from_file_z(const char *file) {
    log_debug("dnsz", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dnsz", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_z(line)) return EXIT_FAILURE;
    }

    return EXIT_SUCCESS;
}

int dns_random_bytes_z(char *dst, int len, const unsigned char *charset,
...
static int dnsz_global_init(struct state_conf *conf) {
    num_questions_z = conf->target_index_num;
    ...
    if (num_questions_z < 1) {
        log_fatal("dnsz", "invalid number of probes for the DNS module: %d",
                  num_questions_z);
    }

    // Setup the global structures
    dns_packets_z     = xmalloc(sizeof(char *) * num_questions_z);
    dns_packet_lens_z = xmalloc(sizeof(uint16_t) * num_questions_z);
    qname_lens_z      = xmalloc(sizeof(uint16_t) * num_questions_z);
    qnames_z          = xmalloc(sizeof(char *) * num_questions_z);
    qtypes_z          = xmalloc(sizeof(uint16_t) * num_questions_z);
    domains_z         = xmalloc(sizeof(char *) * num_questions_z);

    for (int i = 0; i < num_questions_z; i++) {
        domains_z[i] = (char *) default_domain_z;
        qtypes_z[i]  = default_qtype_z;
    }

    // This is xmap boilerplate. Why do I have to write this?
    dns_num_ports_z = conf->source_port_last - conf->source_port_first + 1;
    setup_qtype_str_map_z();
    ...
    if (conf->probe_args &&
        ...
        if (strncasecmp(conf->probe_args, "raw", 3) == 0) {
            label_type_z = DNS_LTYPE_RAW;
            log_debug("dnsz", "raw label prefix");
        } else if (strncasecmp(conf->probe_args, "time", 4) == 0) {
            label_type_z = DNS_LTYPE_TIME;
            log_debug("dnsz", "time label prefix");
        } else if (strncasecmp(conf->probe_args, "random", 6) == 0) {
            label_type_z = DNS_LTYPE_RANDOM;
            log_debug("dnsz", "random label prefix");
        } else if (strncasecmp(conf->probe_args, "str", 3) == 0) {
            label_type_z     = DNS_LTYPE_STR;
            conf->probe_args = c;
            ...
            label_len_z = c - conf->probe_args;
            label_z     = xmalloc(label_len_z);
            strncpy(label_z, conf->probe_args, label_len_z);
            ++c;
            log_debug("dnsz", "label prefix: %s, len: %d", label_z,
                      label_len_z);
        } else {
            log_error("dnsz", dnsz_usage_error);
            return EXIT_FAILURE;
        ...
        if (strncasecmp(conf->probe_args, "text", 4) == 0) {
            if (load_question_from_str_z(c)) return EXIT_FAILURE;
        } else if (strncasecmp(conf->probe_args, "file", 4) == 0) {
            if (load_question_from_file_z(c)) return EXIT_FAILURE;
        } else {
            log_error("dnsz", dnsz_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (index_questions_z < num_questions_z) {
            log_error("dnsz", "more probes than questions configured. Add "
                              "additional probes.");
            return EXIT_FAILURE;
        }
    }

    if (label_type_z == DNS_LTYPE_RAW || label_type_z == DNS_LTYPE_STR)
        return build_global_dns_packets_z(domains_z, num_questions_z);
    else
        return EXIT_SUCCESS;
}

static int dnsz_global_cleanup(UNUSED struct state_conf *xconf,
                               UNUSED struct state_send *xsend,
                               UNUSED struct state_recv *xrecv) {
    if (dns_packets_z) {
        for (int i = 0; i < num_questions_z; i++) {
            if (dns_packets_z[i]) {
                free(dns_packets_z[i]);
            }
        }
        free(dns_packets_z);
    }
    ...
    if (dns_packet_lens_z) {
        free(dns_packet_lens_z);
    }
...
int dnsz_thread_init(void *buf, macaddr_t *src, macaddr_t *gw, void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_z == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth_header(eth_header, src, gw);

    struct ip *ip_header = (struct ip *) (&eth_header[1]);
    uint16_t   ip_len =
        sizeof(struct ip) + sizeof(struct udphdr) + dns_packet_lens_z[0];
    make_ip_header(ip_header, IPPROTO_UDP, ip_len);

    struct udphdr *udp_header = (struct udphdr *) (&ip_header[1]);
    uint16_t       udp_len    = sizeof(struct udphdr) + dns_packet_lens_z[0];
    make_udp_header(udp_header, udp_len);

    char *payload             = (char *) (&udp_header[1]);
    module_dnsz.packet_length = sizeof(struct ether_header) +
                                sizeof(struct ip) + sizeof(struct udphdr) +
                                dns_packet_lens_z[0];
    assert(module_dnsz.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_z[0], dns_packet_lens_z[0]);
...
int dnsz_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    if (label_type_z == DNS_LTYPE_RAW || label_type_z == DNS_LTYPE_STR) {
        // For num_questions_z == 1, we handle this in per-thread init. Do less
        // work
        if (num_questions_z > 1) {
            uint16_t ip_len = sizeof(struct ip) + sizeof(struct udphdr) +
                              dns_packet_lens_z[index];
            make_ip_header(ip_header, IPPROTO_UDP, ip_len);

            uint16_t udp_len = sizeof(struct udphdr) + dns_packet_lens_z[index];
            make_udp_header(udp_header, udp_len);

            char *payload = (char *) (&udp_header[1]);
            *buf_len      = sizeof(struct ether_header) + sizeof(struct ip) +
                       sizeof(struct udphdr) + dns_packet_lens_z[index];

            assert(*buf_len <= MAX_PACKET_SIZE);

            memcpy(payload, dns_packets_z[index], dns_packet_lens_z[index]);
        }
        ...
        udp_header->uh_sport =
            htons(get_src_port(dns_num_ports_z, probe_num, validation));
        ...
        case DNS_LTYPE_RANDOM: {
            ...
            dns_random_bytes_z(new_label, new_label_len - 1,
                               charset_alpha_lower_z, 26, aes);
            new_label[new_label_len - 1] = '\0';
            break;
        }
        default:
            log_fatal("dnsz", dnsz_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (dns_packet_lens_z[index] > DNS_SEND_LEN) {
            log_fatal("dnsz", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_z[index], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }
        ...
        char              *qname_p = dns_packets_z[index] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_z[index] + sizeof(dns_header) +
                                   qname_lens_z[index]);
        ...
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_z[index], qname_lens_z[index]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_z[index]);
        // Set the qclass to The Internet (TM) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x01);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
        ...
        *buf_len      = sizeof(struct ether_header) + sizeof(struct ip) +
        ...
        udp_header->uh_sport =
            htons(get_src_port(dns_num_ports_z, index, validation));
        udp_header->uh_dport = htons(dst_port);
...
void dnsz_print_packet(FILE *fp, void *packet) {
    ...
    fprintf(fp,
            ...
            question_name,
            qtype_strs_z[qtype_qtype_to_strid_z[ntohs(tail_p->qtype)]]);
    fprintf(fp, "------------------------------------------------------\n");

    free(question_name);
}

int dnsz_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip_hdr->ip_p == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_z, validation)) {
            return PACKET_INVALID;
        }
        ...
        // if ip == 172.64.33.143
        if (ip_hdr->ip_src.s_addr == 2401321132) {
        ...
        //            int   ip_type_domain_len = xconf.ipv46_bytes + 2 +
        //            strlen(qname); char *ip_type_domain     =
        //            xmalloc(ip_type_domain_len); memcpy(ip_type_domain, (char
        //            *) &(ip_hdr->ip_src.s_addr),
        //                   xconf.ipv46_bytes);
        //            ip_type_domain[xconf.ipv46_bytes] = (char) (tail_p->qtype
        //            >> 8u); ip_type_domain[xconf.ipv46_bytes + 1] =
        //                (char) (tail_p->qtype & 0xffu);
        //            memcpy(ip_type_domain + xconf.ipv46_bytes + 2, qname,
        //                   strlen(qname));
        //            if (bloom_filter_check_string(
        ...
        // All other header fields should be 0. Except id, which we set
        // per thread. Please recurse as needed.
        dns_header_p->rd = 1; // Is one bit. Don't need htons, recursive
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qname, qname_len);
        // Set the qtype to the same
        tail_p->qtype = dns_type;
        // Set the qclass to The Internet (TM) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x01);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
...
void dnsz_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
    ...
    if (ip_header->ip_p == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_z(
        ...
        // packet prevent this.
        log_fatal("dnsz", "Die. This can only happen if you "
...
probe_module_t module_dnsz = {
    ...
    .port_args       = 1,
    .global_init     = &dnsz_global_init,
    .thread_init     = &dnsz_thread_init,
    .make_packet     = &dnsz_make_packet,
    .print_packet    = &dnsz_print_packet,
    .validate_packet = &dnsz_validate_packet,
    .process_packet  = &dnsz_process_packet,
    .close           = &dnsz_global_cleanup,
    .output_type     = OUTPUT_TYPE_DYNAMIC,
    .fields          = fields,
    .numfields       = sizeof(fields) / sizeof(fields[0]),
    .helptext =
        "This module sends out DNS queries and parses basic responses.\n"
        "Once a successful reply is received, a duplicate query will be "
        "repeated.\n"
