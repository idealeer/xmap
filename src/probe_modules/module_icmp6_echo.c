...
// probe module for performing ICMPv6 echo request (ping) scans

#include "module_icmp6.h"

probe_module_t module_icmp6_echo;
...
static int icmp6_echo_global_init(struct state_conf *conf) {
    ...
    if (strncmp(conf->probe_args, "text", 4) == 0) {
        ...
        if (input_size > ICMP6_MAX_PAYLOAD_LEN) {
            log_error("icmp6",
                      "input file larger than %d bytes and will not fit on the "
                      "wire (%llu bytes provided)",
    ...
    module_icmp6_echo.packet_length = sizeof(struct ether_header) +
                                      sizeof(struct ip6_hdr) + ICMP6_MINLEN +
                                      icmp6_payload_len;
    assert(module_icmp6_echo.packet_length <=
           1500 + sizeof(struct ether_header));
    module_icmp6_echo.pcap_snaplen =
...
static int icmp6_echo_global_cleanup(UNUSED struct state_conf *xconf,
...
static int icmp6_echo_thread_init(void *buf, macaddr_t *src, macaddr_t *gw,
                                  UNUSED void **arg_ptr) {
...
static int icmp6_echo_make_packet(void *buf, UNUSED size_t *buf_len,
...
static int icmp6_echo_validate_packet(const struct ip *ip_hdr, uint32_t len,
                                      int *is_repeat) {
...
static void icmp6_echo_process_packet(const u_char *packet, uint32_t len,
                                      fieldset_t *           fs,
...
probe_module_t module_icmp6_echo = {
    ...
    .port_args       = 0,
    .global_init     = &icmp6_echo_global_init,
    .close           = &icmp6_echo_global_cleanup,
    .thread_init     = &icmp6_echo_thread_init,
    .make_packet     = &icmp6_echo_make_packet,
    .print_packet    = &icmp6_echo_print_packet,
    .process_packet  = &icmp6_echo_process_packet,
    .validate_packet = &icmp6_echo_validate_packet,
    .output_type     = OUTPUT_TYPE_STATIC,
    .fields          = fields,
    .numfields       = sizeof(fields) / sizeof(fields[0]),
    .helptext = "Probe module that sends ICMPv6 echo requests to hosts.\n"
