...
// to discover routers with routing loop

#include "module_icmp6.h"

probe_module_t module_icmp6_echo_tmxd;
...
static int icmp6_echo_timexceed_global_init(struct state_conf *conf) {
    ...
    module_icmp6_echo_tmxd.packet_length = sizeof(struct ether_header) +
                                           sizeof(struct ip6_hdr) +
                                           ICMP6_MINLEN + icmp6_payload_len;
    assert(module_icmp6_echo_tmxd.packet_length <=
           1500 + sizeof(struct ether_header));
    module_icmp6_echo_tmxd.pcap_snaplen =
...
static int
    icmp6_echo_timexceed_global_cleanup(UNUSED struct state_conf *xconf,
...
static int icmp6_echo_timexceed_make_packet(
    ...
    ip6_header->ip6_hlim = ttl - probe_num;
...
static int icmp6_echo_timexceed_validate_packet(
    ...
    // whether repeat: reply <ip, icmp-type>
    char ip_port_str[xconf.ipv46_bytes + 1];
    memcpy(ip_port_str, (char *) &(ip6_header->ip6_src), xconf.ipv46_bytes);
    ip_port_str[xconf.ipv46_bytes] = (char) icmp6_header->icmp6_type;
    if (bloom_filter_check_string(&xrecv.bf, (const char *) ip_port_str,
                                  xconf.ipv46_bytes + 1) == BLOOM_FAILURE) {
...
static void icmp6_echo_timexceed_process_packet(const u_char *packet,
...
static fielddef_t fields[] = {
    {.name = "success",
     .type = "int",
     .desc = "did probe module classify response as a time-exceeded reply"},
    {.name = "clas",
     .type = "string",
...
probe_module_t module_icmp6_echo_tmxd = {
    ...
    .helptext = "Probe module that sends ICMPv6 echo requests to hosts for "
                "discovering routers with routing loop. "
