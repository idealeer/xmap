 ...
 *
 * Usage: xmap -p 53 --probe-module=dnscse --probe-args="raw:text:A,qq.com"
...
// xmap boilerplate
probe_module_t module_dnscse;
static int     dns_num_ports_cse;

const char     default_domain_cse[] = "www.qq.com";
const uint16_t default_qtype_cse    = DNS_QTYPE_A;
const char    *dnscse_usage_error =
...
const unsigned char *charset_alpha_lower_cse =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_cse;
static uint16_t *dns_packet_lens_cse; // Not including udp header
static uint16_t *qname_lens_cse;      // domain_len list
static char    **qnames_cse;          // domain list for query
static uint16_t *qtypes_cse;          // query_type list
static char    **domains_cse;         // domain strs
static int       num_questions_cse   = 0;
static int       index_questions_cse = 0;

const char      default_option_qname_cse[]   = {0x00};
static int      default_option_qname_len_cse = 1;
static uint16_t default_option_udpsize_cse   = 4096;
const char      default_option_rdata_cse[];
static int      default_option_rdata_len_cse = 0;

/* Array of qtypes_cse we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_cse (below)
 * qtype_qtype_to_strid_cse (below, and setup_qtype_str_map_cse())
 */
const char *qtype_strs_cse[]   = {"A",    "NS",    "CNAME", "SOA",      "PTR",
                                  "MX",   "TXT",   "AAAA",  "RRSIG",    "ANY",
                                  "SIG",  "SRV",   "DS",    "DNSKEY",   "TLSA",
                                  "SVCB", "HTTPS", "CAA",   "HTTPSSVC", "OPT"};
const int   qtype_strs_len_cse = 20;

const dns_qtype qtype_strid_to_qtype_cse[] = {
...
int8_t qtype_qtype_to_strid_cse[65536] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_cse() {
    qtype_qtype_to_strid_cse[DNS_QTYPE_A]        = 0;
    qtype_qtype_to_strid_cse[DNS_QTYPE_NS]       = 1;
    qtype_qtype_to_strid_cse[DNS_QTYPE_CNAME]    = 2;
    qtype_qtype_to_strid_cse[DNS_QTYPE_SOA]      = 3;
    qtype_qtype_to_strid_cse[DNS_QTYPE_PTR]      = 4;
    qtype_qtype_to_strid_cse[DNS_QTYPE_MX]       = 5;
    qtype_qtype_to_strid_cse[DNS_QTYPE_TXT]      = 6;
    qtype_qtype_to_strid_cse[DNS_QTYPE_AAAA]     = 7;
    qtype_qtype_to_strid_cse[DNS_QTYPE_RRSIG]    = 8;
    qtype_qtype_to_strid_cse[DNS_QTYPE_ALL]      = 9;
    qtype_qtype_to_strid_cse[DNS_QTYPE_SIG]      = 10;
    qtype_qtype_to_strid_cse[DNS_QTYPE_SRV]      = 11;
    qtype_qtype_to_strid_cse[DNS_QTYPE_DS]       = 12;
    qtype_qtype_to_strid_cse[DNS_QTYPE_DNSKEY]   = 13;
    qtype_qtype_to_strid_cse[DNS_QTYPE_TLSA]     = 14;
    qtype_qtype_to_strid_cse[DNS_QTYPE_SVCB]     = 15;
    qtype_qtype_to_strid_cse[DNS_QTYPE_HTTPS]    = 16;
    qtype_qtype_to_strid_cse[DNS_QTYPE_CAA]      = 17;
    qtype_qtype_to_strid_cse[DNS_QTYPE_HTTPSSVC] = 18;
    qtype_qtype_to_strid_cse[DNS_QTYPE_OPT]      = 19;
}

static uint16_t qtype_str_to_code_cse(const char *str) {
    for (int i = 0; i < qtype_strs_len_cse; i++) {
        if (strcmp(qtype_strs_cse[i], str) == 0)
            return qtype_strid_to_qtype_cse[i];
    }

    return 0;
}

static char    *label_cse      = NULL;
static uint16_t label_len_cse  = 0;
static uint16_t label_type_cse = DNS_LTYPE_RAW;
static uint16_t recursive_cse  = 1;

static uint16_t domain_to_qname_cse(char **qname_handle, const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_cse(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_cse[i] = domain_to_qname_cse(&qnames_cse[i], domains[i]);
        if (domains[i] != (char *) default_domain_cse) {
            free(domains[i]);
        }
        dns_packet_lens_cse[i] =
            sizeof(dns_header) + qname_lens_cse[i] + sizeof(dns_question_tail) +
            default_option_qname_len_cse + sizeof(dns_option_tail) +
            default_option_rdata_len_cse;
        if (dns_packet_lens_cse[i] > DNS_SEND_LEN) {
...
static uint16_t get_name_helper_cse(const char *data, uint16_t data_len,
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dnscse", "_get_name_helper, ptr encountered");
            ...
            log_trace("dnscse", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            uint16_t rec_bytes_consumed = get_name_helper_cse(
            ...
            if (rec_bytes_consumed == 0) {
                log_trace("dnscse", "_get_name_helper OUT. rec level %d failed",
                          recursion_level);
                return 0;
            } else {
                bytes_consumed += 2;
            ...
            log_trace("dnscse", "_get_name_helper, segment 0x%hx encountered",
                      byte);
            ...
            if (byte > name_len) {
                log_warn("dnscse", "Exceeded static name field allocation.");
                return 0;
            }
...
static char *get_name_cse(const char *data, uint16_t data_len,
                          const char *payload, uint16_t payload_len,
                          uint16_t *bytes_consumed) {
    log_trace("dnscse", "call to get_name_cse, data_len: %d", data_len);
    char *name      = xmalloc(MAX_NAME_LENGTH);
    *bytes_consumed = get_name_helper_cse(data, data_len, payload, payload_len,
    ...
    log_trace(
        "dnscse",
        "return success from get_name_cse, bytes_consumed: %d, string: %s",
        *bytes_consumed, name);

    return name;
}

static bool process_response_question_cse(char **data, uint16_t *data_len,
                                          const char *payload,
...
static int load_question_from_str_cse(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (index_questions_cse >= num_questions_cse) {
            log_error("dnscse", "less probes than questions configured. Add "
        ...
        if (label_type_cse == DNS_LTYPE_STR) {
            domains_cse[index_questions_cse] =
                xmalloc(label_len_cse + 1 + domain_len + 1);
            strncpy(domains_cse[index_questions_cse], label_cse, label_len_cse);
            domains_cse[index_questions_cse][label_len_cse] = '.';
            strncpy(domains_cse[index_questions_cse] + label_len_cse + 1,
                    probe_q_delimiter_p + 1, domain_len);
            domains_cse[index_questions_cse][label_len_cse + 1 + domain_len] =
                '\0';
        } else {
            domains_cse[index_questions_cse] = xmalloc(domain_len + 1);
            strncpy(domains_cse[index_questions_cse], probe_q_delimiter_p + 1,
                    domain_len);
            domains_cse[index_questions_cse][domain_len] = '\0';
        }
        ...
        qtypes_cse[index_questions_cse] =
            qtype_str_to_code_cse(strupr(qtype_str));
        if (!qtypes_cse[index_questions_cse]) {
            log_error("dnscse", "incorrect qtype supplied: %s", qtype_str);
            free(qtype_str);
            return EXIT_FAILURE;
        }
        free(qtype_str);
...
static int load_question_from_file_cse(const char *file) {
    log_debug("dnscse", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dnscse", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_cse(line)) return EXIT_FAILURE;
    }

    return EXIT_SUCCESS;
}

int dns_random_bytes_cse(char *dst, int len, const unsigned char *charset,
...
static int dnscse_global_init(struct state_conf *conf) {
    num_questions_cse = conf->target_index_num;
...
int dnscse_thread_init(void *buf, macaddr_t *src, macaddr_t *gw,
                       void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_cse == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth_header(eth_header, src, gw);

    struct ip *ip_header = (struct ip *) (&eth_header[1]);
    uint16_t   ip_len =
        sizeof(struct ip) + sizeof(struct udphdr) + dns_packet_lens_cse[0];
    make_ip_header(ip_header, IPPROTO_UDP, ip_len);

    struct udphdr *udp_header = (struct udphdr *) (&ip_header[1]);
    uint16_t       udp_len    = sizeof(struct udphdr) + dns_packet_lens_cse[0];
    make_udp_header(udp_header, udp_len);

    char *payload               = (char *) (&udp_header[1]);
    module_dnscse.packet_length = sizeof(struct ether_header) +
                                  sizeof(struct ip) + sizeof(struct udphdr) +
                                  dns_packet_lens_cse[0];
    assert(module_dnscse.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_cse[0], dns_packet_lens_cse[0]);
...
int dnscse_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    port_h_t src_port = get_src_port(dns_num_ports_cse, 0, validation);
    uint16_t dns_txid = get_dnsa_txid(validation, 0);

    if (label_type_cse == DNS_LTYPE_RAW || label_type_cse == DNS_LTYPE_STR) {
        // For num_questions_cse == 1, we handle this in per-thread init. Do
        // less work
        if (num_questions_cse > 1) {
...
void dnscse_print_packet(FILE *fp, void *packet) {
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name = get_name_cse(data, data_len, (char *) dns_header_p,
    ...
    dns_option_tail *option_tail_p =
        (dns_option_tail *) (option_qname_p + default_option_qname_len_cse);
    ...
    fprintf(fp,
            ...
            question_name,
            qtype_strs_cse[qtype_qtype_to_strid_cse[(uint16_t) tail_p->qtype]],
...
int dnscse_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip_hdr->ip_p == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_cse, validation)) {
            return PACKET_INVALID;
        }
...
void dnscse_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
    ...
    if (ip_header->ip_p == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_cse(
        ...
        for (int i = 0; i < ntohs(dns_header_p->ancount) && !err; i++) {
            err = process_response_answer_cse(
                &data, &data_len, (char *) dns_header_p, udp_len, list);
        }
        ...
        // packet prevent this.
        log_fatal("dnscse", "Die. This can only happen if you "
...
probe_module_t module_dnscse = {
    ...
    .port_args       = 1,
    .global_init     = &dnscse_global_init,
    .thread_init     = &dnscse_thread_init,
    .make_packet     = &dnscse_make_packet,
    .print_packet    = &dnscse_print_packet,
    .validate_packet = &dnscse_validate_packet,
    .process_packet  = &dnscse_process_packet,
    .close           = &dnscse_global_cleanup,
