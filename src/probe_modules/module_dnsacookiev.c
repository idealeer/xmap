...
int dnsacookiev_validate_packet(const struct ip *ip_hdr, uint32_t len,
   ...
   if (ip_hdr->ip_p == IPPROTO_UDP) {
       if ((4 * ip_hdr->ip_hl + sizeof(struct udphdr) +
            sizeof(dns_header)) > len) {
           // buffer not large enough to contain expected udp+dns
           // header
       ...
       // DNS can return ICMP Destination unreach
       // IP( ICMP( IP( UDP (DNS) ) ) ) for a destination unreach
       ...
       udp_len        = ntohs(udp_inner_header->uh_ulen);
   ...
   //    char              *qname     = ((char *) dns_header_p) +
   //    sizeof(dns_header); int                qname_len = strlen(qname) + 1;
   //    dns_question_tail *tail_p =
   //        (dns_question_tail *) ((char *) dns_header_p + sizeof(dns_header)
   //        +
   //                               qname_len);
   ...
   //    ip_type_domain[xconf.ipv46_bytes + 1] = (char) (tail_p->qtype &
   //    0xffu); memcpy(ip_type_domain + xconf.ipv46_bytes + 2, qname,
   //    strlen(qname)); if (bloom_filter_check_string(&xrecv.bf, (const char
   //    *) ip_type_domain,
   ...
   dns_question_tail *tail_p = (dns_question_tail *) (data + bytes_consumed);

   int   ip_type_domain_len = xconf.ipv46_bytes + 2 + strlen(qname);
   char *ip_type_domain     = xmalloc(ip_type_domain_len);
   memset(ip_type_domain, 0, ip_type_domain_len);
   memcpy(ip_type_domain, (char *) &(ip_hdr->ip_src.s_addr),
          xconf.ipv46_bytes);
   ip_type_domain[xconf.ipv46_bytes]     = (char) (tail_p->qtype >> 8u);
...
void dnsacookiev_process_packet(const u_char *packet, uint32_t len,
   ...
   if (ip_header->ip_p == IPPROTO_UDP) {
       ...
       for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
           err = process_response_question_acookiev(
       ...
       for (int i = 0; i < ntohs(dns_header_p->ancount) && !err; i++) {
           err = process_response_answer_acookiev(
               &data, &data_len, (char *) dns_header_p, udp_len, list);
       }
       ...
       // packet prevent this.
       log_fatal("dnsacookiev", "Die. This can only happen if you "
...
probe_module_t module_dnsacookiev = {
   ...
   .port_args       = 1,
   .global_init     = &dnsacookiev_global_init,
   .thread_init     = &dnsacookiev_thread_init,
   .make_packet     = &dnsacookiev_make_packet,
   .print_packet    = &dnsacookiev_print_packet,
   .validate_packet = &dnsacookiev_validate_packet,
   .process_packet  = &dnsacookiev_process_packet,
   .close           = &dnsacookiev_global_cleanup,
   ...
   .helptext =
       ...
       "Besides, EDNS with variable cookie is appended to the tail.\n"
