...
static int build_global_dns_packets_6aie(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        ...
        dns_packet_lens_6aie[i] =
            sizeof(dns_header) + qname_lens_6aie[i] +
...
static bool process_response_question_6aie(char **data, uint16_t *data_len,
    ...
    if (qtype > MAX_QTYPE ||
        qtype_qtype_to_strid_6aie[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(
            qfs, "qtype_str",
            (char *) qtype_strs_6aie[qtype_qtype_to_strid_6aie[qtype]], 0);
    }
...
static bool process_response_answer_6aie(char **data, uint16_t *data_len,
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name =
                get_name_6aie(rdata + 2, rdlength - 2, payload, payload_len,
                              &rdata_bytes_consumed);
            if (rdata_name == NULL) {
                fs_add_uint64(afs, "rdata_is_parsed", 0);
                fs_add_binary(afs, "rdata", rdlength, rdata, 0);
...
static int load_question_from_str_6aie(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (label_type_6aie == DNS_LTYPE_STR) {
            ...
            strncpy(domains_6aie[index_questions_6aie], label_6aie,
                    label_len_6aie);
    ...
    if (label_type_6aie == DNS_LTYPE_RAW || label_type_6aie == DNS_LTYPE_STR) {
        ...
        if (num_questions_6aie > 1) {
            ...
            *buf_len = sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
            ...
            memcpy(payload, dns_packets_6aie[index],
        ...
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_6aie[index] +
                                   sizeof(dns_header) + qname_lens_6aie[index]);
        char *option_qname_p =
