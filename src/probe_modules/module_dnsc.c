...
int dnsc_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    port_h_t src_port = get_src_port(dns_num_ports_c, 0, validation);
    uint16_t dns_txid = get_dns_txid(validation);

    if (label_type_c == DNS_LTYPE_RAW || label_type_c == DNS_LTYPE_STR) {
        // For num_questions_c == 1, we handle this in per-thread init. Do less
        // work
        if (num_questions_c > 1) {
        ...
        case DNS_LTYPE_RANDOM: {
            ...
            dns_random_bytes_c(new_label, 8, charset_alpha_lower_c, 26, aes);
            new_label[8] = '\0';
            break;
        }
        ...
        default:
            log_fatal("dnsc", dnsc_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (dns_packet_lens_c[index] > DNS_SEND_LEN) {
            log_fatal("dnsc", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_c[index], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }
...
int dnsc_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip_hdr->ip_p == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_c, validation)) {
            return PACKET_INVALID;
        }
...
void dnsc_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
    ...
    if (ip_header->ip_p == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_c(
        ...
        // packet prevent this.
        log_fatal("dnsc", "Die. This can only happen if you "
...
probe_module_t module_dnsc = {
    ...
    .port_args       = 1,
    .global_init     = &dnsc_global_init,
    .thread_init     = &dnsc_thread_init,
    .make_packet     = &dnsc_make_packet,
    .print_packet    = &dnsc_print_packet,
    .validate_packet = &dnsc_validate_packet,
    .process_packet  = &dnsc_process_packet,
    .close           = &dnsc_global_cleanup,
