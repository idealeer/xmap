...
char *get_default_iface(void) {
    ...
    if (iface == NULL) {
        log_error("get_gateway-linux", "could not detect default network interface "
...
int get_iface_ip(const char *iface_, uint8_t *ip, int ipv46_flag) {
    ...
    if (!strncmp(iface_, ZCPREFIX, ZCPREFIX_LEN))
        iface = iface_ + ZCPREFIX_LEN;
        ...
        if (!strcmp(iface, ifa->ifa_name)) {
            switch (ipv46_flag) {
                ...
                case IPV6_FLAG: {
                    ...
                    if ((ip6_addr[0] >> 5u) != 0x01)
                        continue; // global unicast
                    int j;
                    for (j = 0; j < 16; j++)
                        ip[j] = ip6_addr[j];
                    return EXIT_SUCCESS;
...
int send_nl_req(uint16_t msg_type, uint32_t seq, void *payload,
    ...
    if (sock < 0) {
        log_error("get_gateway-linux", "unable to get socket: %s", strerror(errno));
        return -1;
    }
...
int get_hw_addr(const uint8_t *gw_ip, const char *iface, unsigned char *hw_mac) {
    struct ndmsg req;
                ...
                case NDA_LLADDR:
                    if (RTA_PAYLOAD(rt_attr) != IFHWADDRLEN) {
                        ...
                        log_error("get_gateway-linux",
                                  ...
                                  " If you are using a VPN, supply the --iplayer flag (and "
                                  "provide an"
                                  " interface via -i)",
                                  RTA_PAYLOAD(rt_attr));
                        return EXIT_FAILURE;
