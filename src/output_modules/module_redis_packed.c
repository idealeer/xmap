...
int redismodule_init(struct state_conf *conf, char **fields, int fieldlens) {
    ...
    if (field_index < 0) {
        log_fatal("redis-module", "saddr_raw not included in output-fields");
    }
    ...
    if (conf->output_args) {
        ...
        connect_string = strdup("local:///tmp/redis.sock/xmap_output");
    }

    if (redis_parse_connstr(connect_string, &rconf) != PMAP_REDIS_SUCCESS) {
...
output_module_t module_redis = {.name                    = "redis-packed",
                                ...
                                .helptext =
                                    "Flushes to redis the ipv46 address as "
                                    "packed binary integer in network order\n"};
